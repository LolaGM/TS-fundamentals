...
<h2>1. Description</h2>
<p>Exercises and practice for TYPESCRIPT following lesson 3 from UDEMY online ANGULAR course during internship at Bosonit company.</p>
...
<h2>2. Contents</h2>
<p>Practice TYPESCRIPT in its first steps to ANGULAR.</p>
...
* When you finish installing the node_modules, then we can run the project with the following command

```
npm start
```
For that to work, remember that you must execute that command in the same directory where the ```package.json``` is located.

<h3></h3> Change port</h3>
...
They simply change the port for the one you need and that's it. (of course they save the changes before running ```npm start``` again)
